...
int main()
{
  ...
  double pi, hr, ht, ri, ri2, theta_j, M, A, ex;
  ...
  for (n = 1; n <= nMax; n++) {
	...
	if(e2 < A)
      ...
      printf("n=%d, maxnorm=%g, t=%g, min(u(1,j))= %g,誤差=%g\n",
             n, maxnorm(Nr + 1, Nt + 1, u), t, A, M);
    }
    else {
      ex = exactu(0.0, 0.0, t);
      M = fabs(u[0][0] - ex);
...
{
  return exp(- mu01 * mu01 * t) * j0(mu01 * r)+ exp(- mu11 * mu11 * t) * j1(mu11 * r) * (cos(theta) + sin(theta));
}

/* 初期データ */
double u0(double r, double theta)
{
  return j0(mu01 * r) + j1(mu11 * r) * (cos(theta) + sin(theta));
}
