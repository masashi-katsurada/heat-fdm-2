/*
 * heat2n-adi.c --- 2次元長方形領域における熱伝導方程式 (Neumann 条件) を
 *                  ADI 法で解く。
 *
 *   コンパイルするには
 *     gcc -c trid-lu.c
 *   としてtrid-lu.o を作っておいて
 *     gcc -o heat2n-adi heat2n-adi.c trid-lu.o -lmatrix -lglscd -lX11 -lm
 *   または
 *     ccmg heat2n-adi.c trid-lu.c
 */
...
double u0(double, double), beta(double, double);
...
int main()
{
    ...
    alx[0] = 0; adx[0] = c1; aux[0] = 2 * c2;
    alx[N_x] = 2 * c2; adx[N_x] = c1; aux[N_x] = 0;
    ...
    aly[0] = 0; ady[0] = c5; auy[0] = 2 * c6;
    aly[N_y] = 2 * c6; ady[N_y] = c5; auy[N_y] = 0;

    kmax = Tmax / tau + 0.5;
    printf("skip = %d\n", skip);

    /* LU分解 */
    trilu(N_y + 1, aly, ady, auy);
    trilu(N_x + 1, alx, adx, aux);
	...
	for (j = 0; j <= N_y; j++) {
	    ...
	    for (i = 0; i <= N_x; i++) {
	      ...
	      if (j == 0)
		ff[i] = c3 * U[i][0]
		  + 2 * lambda_y * (U[i][1] + h_y * beta(x, c));
	      else if (j == N_y)
		ff[i] = c3 * U[i][N_y]
		  + 2 * lambda_y * (U[i][N_y - 1] + h_y * beta(x, d));
	      else
		ff[i] = c3 * U[i][j] + lambda_y * (U[i][j + 1] + U[i][j - 1]);
	    }
	    ff[0]   += 2.0 * lambda_x * h_x * beta(a, y);
	    ff[N_x] += 2.0 * lambda_x * h_x * beta(b, y);
	    /* 連立1次方程式を解く */
	    trisol(N_x + 1, alx, adx, aux, ff);
	    /* 解を記憶する */
	    for (i = 0; i <= N_x; i++)
		newU[i][j] = ff[i];
	}
	...
	for (i = 0; i <= N_x; i++) {
	    ...
	    for (j = 0; j <= N_y; j++) {
	      ...
	      if (i == 0)
		gg[j] = c7 * newU[0][j]
		  + 2 * lambda_x * (newU[1][j] + h_x * beta(a, y));
	      else if (i == N_x)
		gg[j] = c7 * newU[N_x][j]
		  + 2 * lambda_x * (newU[N_x - 1][j] + h_x * beta(b, y));
	      else
		gg[j] = c7 * newU[i][j]
		  + lambda_x * (newU[i + 1][j] + newU[i - 1][j]);
	    }
	    gg[0]   += 2.0 * lambda_y * h_y * beta(x, c);
	    gg[N_y] += 2.0 * lambda_y * h_y * beta(x, d);
	    /* 連立1次方程式を解く */
	    trisol(N_y + 1, aly, ady, auy, gg);
	    /* 解を記憶する */
	    for (j = 0; j <= N_y; j++)
		U[i][j] = gg[j];
	}
