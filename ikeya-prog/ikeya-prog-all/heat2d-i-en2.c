/*
 * heat2d-i-en.c ---円盤状の2次元熱方程式をθ法で解く
 *                
 *   コンパイルするには
 *     gcc -c lu.c
 *     gcc -c call_gnuplot.c
 *  
 *     ccmg heat2d-i-en.c lu.c gnuplot.c  
 */
#include <stdio.h>
#include <math.h>
#include <matrix.h>
...
/* 原点のコピーの係数 */
 ...
 /* 内点での係数 */
	...
	A[L][L-1] = - theta*lambda_r*(1.0 - h_r/(2.0*ri));
        A[L][psi(i,N_p-1)] =  - theta * lambda_p / ri2;
        A[L][L+mm] = - theta * lambda_p / ri2;
	                     /* j=0の時L-mm<0のためL-mmをpsi(i.Nθ-1)とする */
	}
      else if(j == N_p-1){
	L = psi(i,j);
	A[L][L] = 1.0 + 2.0*theta*lambda_r + (2.0*theta*lambda_p) / ri2;
	...
	A[L][L-1] = - theta*lambda_r*(1.0 - h_r/(2.0*ri));
        A[L][L-mm] = - theta*lambda_p / ri2;
        A[L][psi(i,0)] = - theta * lambda_p / ri2;
                             /* j=Nθ-1の時L+mm>NNのためL+mmをpsi(i.0)とする */
	}
	 else{
	   L = psi(i,j);
	A[L][L] = 1.0 + 2.0*theta*lambda_r + (2.0*theta*lambda_p) / ri2;
