 ...
 *  
 *     ccmg ss.c bandlu.c call_gnuplot.c  
 */
  ...
  for (i = 1; i< N_r; i++) {
    ...
    for (j = 0; j <= N_p-1; j++) {
      ...
      A[L][L] = 1.0 + (ri3 * theta * lambda_r) / ri
        + (ri4 * theta * lambda_r) / ri
        + (2.0 * theta * lambda_p) / ri2;
  ...
  for (n = 1; n <= nMax; n++) {
    ...
    for (i = 1; i < N_r; i++) {
      ...
      for (j = 0; j<= N_p-1; j++) {
        ...
        if (jm2 == N_p)
          jm2 = 0;
        B[L] = (1.0 - (ri3*(1-theta)*lambda_r)/ri
                - (ri4*(1-theta)*lambda_r)/ri
